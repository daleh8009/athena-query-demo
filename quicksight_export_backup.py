...
class QuickSightExporter:
    ...
    def generate_dataset_name(self, user_prompt, query_description=""):
        """Generate a clean dataset name from user prompt"""
        base_name = user_prompt or query_description or "Athena Query Results"
    ...
    def export_to_quicksight(self, sql_query, user_prompt, query_description=""):
        """Main export function"""
        try:
            # Generate dataset details
            dataset_name = self.generate_dataset_name(user_prompt, query_description)
...
def render_quicksight_export_ui(sql_query, user_prompt, query_description="", config=None):
    ...
    # 4-button workflow - no redundant SQL display since backend uses RelationalTable
    ...
    with col2:
        if st.session_state.qs_export_success:
            ...
            <a href="{st.session_state.qs_urls.get('dataset', '')}" target="_blank" 
               style="display: inline-block; padding: 0.5rem 1rem; background-color: #1e1e1e; 
    ...
    with col3:
        if st.session_state.qs_export_success:
            ...
            <a href="{st.session_state.qs_urls.get('create_analysis', '')}" target="_blank" 
               style="display: inline-block; padding: 0.5rem 1rem; background-color: #ff6b35; 
                      color: white; text-decoration: none; border-radius: 0.5rem; 
                      text-align: center; width: 100%; box-sizing: border-box;">
                📈 Create Analysis
            </a>
    ...
    if export_button:
        with st.spinner("🔄 Creating QuickSight dataset..."):
            ...
            result = exporter.export_to_quicksight(sql_query, user_prompt, query_description)
        
        if result['success']:
            st.success(f"✅ {result['message']}")
            st.session_state.qs_export_success = True
            st.session_state.qs_urls = result['urls']
            st.rerun()  # Refresh to show active buttons
        else:
